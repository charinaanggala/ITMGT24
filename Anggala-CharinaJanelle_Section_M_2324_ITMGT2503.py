...
with open("script.json","r") as file: 
        ...
        #script = json.dumps(dictionary_data, file)
...
for i in range(len(jsondata)): 
    for j in range(len(jsondata[i])):
        jsondata[i][j] = str(jsondata[i][j])

for i in range(len(jsondata)): 
    for j in range(len(jsondata[i])):
        jsondata[i][j] = jsondata[i][j].upper()
...
for i in range(len(unecessary_punctuation)): 
    for j in range(len(jsondata[i])):
        jsondata[i]
...
for i in range(18000//1000 + 1): 
    for j in range(18000//600): 
        for k in range(18000//750):
            Gold = 1000*i + 600*j+ 750*k
            Lumber = 500*i + 750*j + 300*k
            Food = 50*optimal_F + 45*optimal_R + 75*optimal_C

            if Gold <= 18000 and Lumber <= 12000 and Food <=1500:
                if optimal_Z > max:
                    max = optimal_Z
                    optimal_Z = 10*i + 12*j + 16*k
                    optimal_F = i
                    optimal_R = j
                    optimal_C = k
                    i = i +1
                    ...
                    break 
print(max, optimal_F, optimal_R , optimal_C )
...
def Collatz(start_number):
    ...
    '''
    dict = {'sequence':[], 'collatz_length':0, 'max_collatz':0, 'sequence_sum':0}
    lst = []
    num = start_number
    lst.append(start_number)
    while num != 1:
        if num % 2 == 0:
            num = num/2
    ...
    dict['max_collatz'] = max
    dict['sequence_sum'] = sum
    return dict
Collatz(5)
...
def Collatz_winner(number_list):
    ...
    for i in range(len(result)):
        if max < result[i]['max_collatz'] and length < result[i]['collatz_length']:
            winner = result[i]['sequence'][0]
                
    return winner
    
print(Collatz_winner(range(50,100)))
